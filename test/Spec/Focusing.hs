...
testHelper :: ((FilePath, ModuleName), Either (NonEmpty Error) TST.Module) -> SpecWith ()
testHelper ((example, mn),decls) = describe ("Focusing the program in  " <> example <> " typechecks.") $ 
  ...
  case mn `lookup` pendingFiles of
    ...
    Nothing     -> 
      case decls of
        ...
        Right decls -> do
          let focusedDecls :: CST.Module = runUnresolveM $ unresolve $ embedCore $ embedTST $ focus decls
