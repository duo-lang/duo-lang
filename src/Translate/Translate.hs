...
compile (Dtor _ xt t args) =
  let
    cmd = Apply () Nothing (compile t) (XtorCall () CnsRep xt $ MkXtorArgs (compile <$> args) [FreeVar () CnsRep resVar])
  in
...
compile (Match _ t cases)   =
  let
    compileMatchCase (MkACase _ xt args t) = MkSCase xt (Twice (args) [])   $ Apply () Nothing (compile t) (FreeVar () CnsRep resVar)
    cmd = Apply () Nothing (compile t) (XMatch () CnsRep Nominal  (compileMatchCase <$> cases))
  in
    MuAbs () PrdRep Nothing $ commandClosingSingle CnsRep resVar $ shiftCmd cmd
...
compile (Comatch _ cocases) =
  let
    compileComatchCase (MkACase _ xt args t) = MkSCase xt (Twice (const Nothing <$> args) [Nothing]) $ Apply () Nothing (compile t) (BoundVar () CnsRep (0,0))
  in
...
compileCmd (Apply _ cc prd cns) = Apply () cc (compileSTerm prd) (compileSTerm cns)
