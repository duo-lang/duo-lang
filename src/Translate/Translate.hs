...
compile (Match _ t cases)   = MuAbs () PrdRep Nothing $ 
...
compileArgs :: [ATerm ext] -> [STerm Cns ()] -> XtorArgs ()
compileArgs args' cnsLst = MkXtorArgs (compile <$> args') cnsLst

aToSCase :: PrdCnsRep pc -> ACase ext -> SCase ()
-- we want to compile: C (args) => t
-- C (args) => (compile t) >> k 
aToSCase PrdRep (MkACase _ xt args t) = MkSCase xt (Twice (const Nothing <$> args) [])   $ Apply () (compile t) (BoundVar () CnsRep (1,0))
-- we want to compile: D(args) => t
-- D(args)[k] => (compile t) >> k 
aToSCase _      (MkACase _ xt args t) = MkSCase xt (Twice (const Nothing <$> args) [Nothing]) $ Apply () (compile t) (BoundVar () CnsRep (0,0))
...
--Dtor
shift k (MuAbs _ PrdRep Nothing (Apply _ t (XtorCall _ CnsRep xt (MkXtorArgs args' [BoundVar _ CnsRep (j,0)])))) =
  MuAbs () PrdRep Nothing $ Apply () (shift k t) $ XtorCall () CnsRep xt $ MkXtorArgs (shift k <$> args') [BoundVar () CnsRep (j,0)]
--Match
shift k (MuAbs _ PrdRep Nothing (Apply _ t (XMatch _ CnsRep Nominal cases))) =
  MuAbs () PrdRep Nothing $ Apply () (shift (k+1) t) $ XMatch () CnsRep Nominal $ shiftCase PrdRep (k+1) <$> cases
--Comatch
...
shiftCase _      k (MkSCase xt (Twice args [Nothing]) (Apply _ t (BoundVar _ CnsRep (j,0)))) =
  MkSCase xt (Twice args [Nothing]) $ Apply () (shift (k+1) t) (BoundVar () CnsRep (j,0))
