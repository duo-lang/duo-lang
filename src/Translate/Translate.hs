...
--Dtor
shift k (MuAbs PrdRep () (Apply _ t (XtorCall CnsRep xt (MkXtorArgs args' [BoundVar CnsRep (j,0)])))) =
  MuAbs PrdRep () $ Apply () (shift k t) $ XtorCall CnsRep xt $ MkXtorArgs (shift k <$> args') [BoundVar CnsRep (j,0)]
--Match
shift k (MuAbs PrdRep () (Apply _ t (XMatch CnsRep Nominal cases))) =
...
shiftCase PrdRep k (MkSCase xt (Twice args []) (Apply _ t (BoundVar CnsRep (j,0)))) =
  MkSCase xt (Twice args []) (Apply () (shift (k+1) t) (BoundVar CnsRep (j,0)))
-- shift SCase produced through Comatch
shiftCase _      k (MkSCase xt (Twice args [()]) (Apply _ t (BoundVar CnsRep (j,0)))) =
  MkSCase xt (Twice args [()]) $ Apply () (shift (k+1) t) (BoundVar CnsRep (j,0))
