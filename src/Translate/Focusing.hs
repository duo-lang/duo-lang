...
focusXtor' eo CnsRep ty ns xt [] pcterms' = Apply defaultLoc ApplyAnnotOrig  (MkEo eo) (FreeVar defaultLoc PrdRep (TyFlipPol PosRep ty) alphaVar)
                                                                           (Xtor defaultLoc XtorAnnotOrig CnsRep ty ns xt (MkSubstitution $ reverse pcterms'))
focusXtor' eo PrdRep ty ns xt [] pcterms' = Apply defaultLoc ApplyAnnotOrig  (MkEo eo) (Xtor defaultLoc XtorAnnotOrig PrdRep ty ns xt (MkSubstitution $ reverse pcterms'))
...
focusXtor' eo pc     ty ns xt (PrdTerm                                 prd:pcterms) pcterms' =
                                                              ...
                                                              in
                                                                  Apply defaultLoc ApplyAnnotOrig  (MkEo eo) (focus eo prd) (MuAbs defaultLoc MuAnnotOrig CnsRep (TyFlipPol NegRep tprd) Nothing cmd)
...
focusXtor' eo pc     ty ns xt (CnsTerm                                 cns:pcterms) pcterms' =
                                                              ...
                                                              in Apply defaultLoc ApplyAnnotOrig  (MkEo eo) (MuAbs defaultLoc MuAnnotOrig PrdRep (TyFlipPol PosRep tcns) Nothing cmd) (focus eo cns)
...
focusPrimOp eo op (PrdTerm prd:pcterms) pcterms' =
    ...
    in
        Apply defaultLoc ApplyAnnotOrig (MkEo eo) (focus eo prd) (MuAbs defaultLoc MuAnnotOrig CnsRep (TyFlipPol NegRep (getTypeTerm prd)) Nothing cmd)
...
focusPrimOp eo op (CnsTerm cns:pcterms) pcterms' =
    ...
    in
        Apply defaultLoc ApplyAnnotOrig  (MkEo eo) (MuAbs defaultLoc MuAnnotOrig PrdRep (TyFlipPol PosRep (getTypeTerm cns)) Nothing cmd) (focus eo cns)

-- | Invariant:
-- The output should have the property `isFocusedCmd cmd`.
instance Focus Command where
  focus :: EvaluationOrder -> Command -> Command
  focus eo (Apply loc _annot _kind prd cns) = Apply loc ApplyAnnotOrig (MkEo eo) (focus eo prd) (focus eo cns)
  ...
  focus eo (Print loc prd cmd) = Apply loc ApplyAnnotOrig (MkEo eo) (focus eo prd)
  ...
  focus eo (Read loc cns) = Apply loc ApplyAnnotOrig (MkEo eo) (MuAbs loc MuAnnotOrig PrdRep (TyFlipPol PosRep (getTypeTerm cns)) Nothing (Read loc (BoundVar loc CnsRep (getTypeTerm cns) (0,0))))
                                                          (focus eo cns)
  focus eo (PrimOp _ op subst) = focusPrimOp eo op subst.unSubstitution []

  isFocused :: EvaluationOrder -> Command -> Maybe Command
  isFocused eo (Apply loc _annot _kind prd cns) = Apply loc ApplyAnnotOrig (MkEo eo) <$> isFocused eo prd <*> isFocused eo cns
