...
focusTerm eo (Xtor _ _annot pcrep ty ns xt subst) = focusXtor eo pcrep ns xt subst
...
focusXtor eo PrdRep ns xt subst =
    MuAbs defaultLoc MuAnnotOrig PrdRep _ Nothing (commandClosing [(Cns, alphaVar)] (shiftCmd ShiftUp (focusXtor' eo PrdRep ns xt subst [])))
focusXtor eo CnsRep ns xt subst =
    MuAbs defaultLoc MuAnnotOrig CnsRep _ Nothing (commandClosing [(Prd, alphaVar)] (shiftCmd ShiftUp (focusXtor' eo CnsRep ns xt subst [])))


focusXtor' :: EvaluationOrder -> PrdCnsRep pc -> NominalStructural -> XtorName -> [PrdCnsTerm] -> [PrdCnsTerm] -> Command
focusXtor' eo CnsRep ns xt [] pcterms' = Apply defaultLoc ApplyAnnotOrig  (Just (CBox eo)) (FreeVar defaultLoc PrdRep _ alphaVar)
                                                                           (Xtor defaultLoc XtorAnnotOrig CnsRep _ ns xt (reverse pcterms'))
focusXtor' eo PrdRep ns xt [] pcterms' = Apply defaultLoc ApplyAnnotOrig  (Just (CBox eo)) (Xtor defaultLoc XtorAnnotOrig PrdRep _ ns xt (reverse pcterms'))
                                                                           (FreeVar defaultLoc CnsRep _ alphaVar)
...
focusXtor' eo pc     ns xt (PrdTerm                                 prd:pcterms) pcterms' =
                                                              let
                                                                  ...
                                                                  cmd = commandClosing [(Prd,var)]  (shiftCmd ShiftUp (focusXtor' eo pc ns xt pcterms (PrdTerm (FreeVar defaultLoc PrdRep _ var) : pcterms')))
                                                              in
                                                                  Apply defaultLoc ApplyAnnotOrig  (Just (CBox eo)) (focusTerm eo prd) (MuAbs defaultLoc MuAnnotOrig CnsRep _ Nothing cmd)
...
focusXtor' eo pc     ns xt (CnsTerm                                 cns:pcterms) pcterms' =
                                                              let
                                                                  var = betaVar (length pcterms') -- OK?
                                                                  cmd = commandClosing [(Cns,var)] (shiftCmd ShiftUp (focusXtor' eo pc ns xt pcterms (CnsTerm (FreeVar defaultLoc CnsRep _ var) : pcterms')))
                                                              in Apply defaultLoc ApplyAnnotOrig  (Just (CBox eo)) (MuAbs defaultLoc MuAnnotOrig PrdRep _ Nothing cmd) (focusTerm eo cns)
...
focusPrimOp eo op (PrdTerm prd:pcterms) pcterms' =
    let
        ...
        cmd = commandClosing [(Prd,var)]  (shiftCmd ShiftUp (focusPrimOp eo op pcterms (PrdTerm (FreeVar defaultLoc PrdRep _ var) : pcterms')))
    in
        Apply defaultLoc ApplyAnnotOrig (Just (CBox eo)) (focusTerm eo prd) (MuAbs defaultLoc MuAnnotOrig CnsRep _ Nothing cmd)
...
focusPrimOp eo op (CnsTerm cns:pcterms) pcterms' =
    let
        var = betaVar (length pcterms')
        cmd = commandClosing [(Cns,var)] (shiftCmd ShiftUp (focusPrimOp eo op pcterms (CnsTerm (FreeVar defaultLoc CnsRep _ var) : pcterms')))
    in
        Apply defaultLoc ApplyAnnotOrig  (Just (CBox eo)) (MuAbs defaultLoc MuAnnotOrig PrdRep _ Nothing cmd) (focusTerm eo cns)
...
focusCmd eo (Print loc prd cmd) = Apply loc ApplyAnnotOrig (Just (CBox eo)) (focusTerm eo prd) 
                                                             (MuAbs loc MuAnnotOrig CnsRep _ Nothing (Print loc (BoundVar loc PrdRep _ (0,0)) (focusCmd eo cmd)))
focusCmd eo (Read loc (isValueTerm eo CnsRep -> Just cns)) = Read loc cns
focusCmd eo (Read loc cns) = Apply loc ApplyAnnotOrig (Just (CBox eo)) (MuAbs loc MuAnnotOrig PrdRep _ Nothing (Read loc (BoundVar loc CnsRep _ (0,0))))
                                                        (focusTerm eo cns)
