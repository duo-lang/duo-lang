...
instance InsertInstance Command where
    ...
    insertInstance (MkInstanceResult m cs) (Method loc mn cn (InstanceUnresolved uv) ty subst) = case M.lookup (uv, cn) m of
        Nothing -> throwOtherError loc [ "Something went wrong. No instance could be resolved for " <> ppPrint cn <> " " <> ppPrint uv ]
        Just (i, _, _) -> Method loc mn cn (InstanceResolved i) ty <$> insertInstance (MkInstanceResult m cs) subst
