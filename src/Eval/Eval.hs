...
evalTermOnce (Read _ cns) = do
  tm <- readM
  return (Just (Apply defaultLoc ApplyAnnotOrig (MkPknd (MkPolyKind [] CBV)) tm cns))
...
evalApplyOnce (MkPknd (MkPolyKind [] CBV)) (MuAbs _ _ PrdRep _ _ cmd) cns@(MuAbs _ _ CnsRep _ _ _) =
...
evalApplyOnce MkString (MuAbs _ _ PrdRep _ _ cmd) cns@(MuAbs _ _ CnsRep _ _ _) =
  return (Just (LN.open (MkSubstitution [CnsTerm cns]) cmd))
evalApplyOnce (MkPknd (MkPolyKind [] CBN)) prd@(MuAbs _ _ PrdRep _ _ _) (MuAbs _ _ CnsRep _ _ cmd) =
