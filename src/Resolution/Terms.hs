...
analyzePattern dc (CST.XtorPat loc xt args) = do
  ...
  case res of
    (MethodNameResult _cn _arity) -> throwOtherError ["Expected xtor but found method " <> unXtorName xt]
    (XtorNameResult dc' _ns arity) -> do
      -- Check whether the Xtor is a Constructor/Destructor as expected.
      case (dc,dc') of
        (Codata,Data  ) -> throwOtherError ["Expected a destructor but found a constructor"]
        (Data  ,Codata) -> throwOtherError ["Expected a constructor but found a destructor"]
        (Data  ,Data  ) -> pure ()
        (Codata,Codata) -> pure ()
      -- Analyze the pattern
...
analyzeInstancePattern (CST.XtorPat loc xt args) = do
  ...
  case res of
    ...
    (XtorNameResult _dc _ns _arity) -> throwOtherError ["Expected method but found Xtor " <> unXtorName xt]
        -- Analyze the pattern
  -- Check whether the number of arguments in the given binding site
  -- corresponds to the number of arguments specified for the constructor/destructor.
...
analyzeCases dc cases = do
  ...
  if | all isExplicitCase cases' -> pure $ ExplicitCases    $ fromExplicitCase <$> cases'
     ...
     | otherwise -> throwOtherError ["Cases mix the use of both explicit and implicit patterns."]
...
analyzeInstanceCase CST.MkTermCase { tmcase_loc, tmcase_pat, tmcase_term } = do
  ...
  case analyzedPattern of
    ...
    _ -> throwOtherError ["Should be unreachable"]
...
resolveCommand (CST.Xtor loc xtor _arity) = do
  ...
  case res of
    ...
    (MethodNameResult _cn _ar) -> throwOtherError ["Method calls not implemented yet"]
  ...
  case length (filter isStarT subst) of
    ...
    1 -> do
      ...
      case span (not . isStarT . snd) zipped of
        ...
        _ -> throwOtherError ["Compiler bug in analyzeSubstitution"]
...
resolveTerm PrdRep (CST.Xtor loc xtor subst) = do
  ...
  case res of
    (XtorNameResult dc ns ar) -> do
      ...
      when (dc /= Data) $
               throwOtherError ["The given xtor " <> ppPrint xtor <> " is declared as a destructor, not a constructor."]
      analyzedSubst <- analyzeSubstitution loc xtor ar subst
      subst' <- case analyzedSubst of
          ExplicitSubst es -> return (map snd es)
          ImplicitSubst {} ->  throwOtherError ["The substitution in a constructor call cannot contain implicit arguments"]
      pctms <- resolveTerms loc ar subst'
      pure $ RST.Xtor loc PrdRep ns xtor pctms
    (MethodNameResult _cn _ar) -> throwOtherError ["Xtor expected, but found Method"]
  ...
  case res of
    (XtorNameResult dc ns ar) -> do
      when (length ar /= length subst) $
               throwError $ LowerError (Just loc) (XtorArityMismatch xtor (length ar) (length subst)) :| []
      when (dc /= Codata) $
               throwOtherError ["The given xtor " <> ppPrint xtor <> " is declared as a constructor, not a destructor."]
      analyzedSubst <- analyzeSubstitution loc xtor ar subst
      subst' <- case analyzedSubst of
          ExplicitSubst es -> return (map snd es)
          ImplicitSubst {} ->  throwOtherError ["The substitution in a constructor call cannot contain implicit arguments"]
...
resolveTerm rep (CST.Semi loc xtor subst tm) = do
  ...
  case analyzedSubst of
    ExplicitSubst _explicitSubst -> do
      throwOtherError ["The substitution in a Semi must contain at least one implicit argument"]
    ImplicitSubst subst1 Prd subst2 -> do
      case rep of
        PrdRep ->
          throwOtherError ["Tried to resolve Semi to a producer, but implicit argument stands for a producer"]
        CnsRep -> do
...
resolveTerm rep (CST.Dtor loc xtor tm subst) = do
  ...
  case analyzedSubst of
    ExplicitSubst _explicitSubst -> do
      throwOtherError ["The substitution in a dtor must contain at least one implicit argument"]
    ImplicitSubst subst1 Prd subst2 -> do
      case rep of
        PrdRep -> do
          throwOtherError ["Tried to resolve Dtor to a producer, but implicit argument stands for a producer"]
        CnsRep -> do
          subst1' <- forM subst1 $ uncurry resolvePrdCnsTerm
          subst2' <- forM subst2 $ uncurry resolvePrdCnsTerm
    ...
    ImplicitSubst subst1 Cns subst2 -> do
      case rep of
        ...
        CnsRep -> do
          throwOtherError ["Tried to resolve Dtor to a consumer, but implicit argument stands for consumer"]
---------------------------------------------------------------------------------
-- Case / Cocase
---------------------------------------------------------------------------------
resolveTerm PrdRep (CST.Case loc _) =
  throwOtherError ["Cannot resolve pattern match to a producer."]
resolveTerm CnsRep (CST.Cocase loc _) =
  throwOtherError ["Cannot resolve copattern match to a consumer."]
...
resolveTerm PrdRep (CST.CaseOf loc t cases)  = do
  ...
  case intermediateCases of
    ...
    ImplicitCases _rep _implicitCases ->
      throwOtherError ["Cannot resolve case-of with implicit cases to producer."]
resolveTerm PrdRep (CST.CocaseOf loc t cases)  = do
  ...
  case intermediateCases of
    ...
    ImplicitCases _rep _implicitCases ->
      throwOtherError ["Cannot resolve cocase-of with implicit cases to producer"]
resolveTerm CnsRep (CST.CaseOf loc t cases) = do
  ...
  case intermediateCases of
    ...
    ImplicitCases _rep _implicitCases ->
      throwOtherError ["Cannot resolve case-of with implicit cases to consumer."]
resolveTerm CnsRep (CST.CocaseOf loc t cases) = do
  ...
  case intermediateCases of
    ...
    ImplicitCases _rep _implicitCases ->
      throwOtherError ["Cannot resolve cocase-of with implicit cases to consumer"]
---------------------------------------------------------------------------------
-- Literals
---------------------------------------------------------------------------------
...
resolveTerm CnsRep (CST.PrimLitI64 loc _) =
  throwOtherError ["Cannot resolve primitive literal to a consumer."]
...
resolveTerm CnsRep (CST.NatLit loc _ns _i) =
  throwOtherError ["Cannot resolve NatLit to a consumer."]
---------------------------------------------------------------------------------
...
resolveTerm PrdRep (CST.CoLambda loc _fv _tm) =
  throwOtherError ["Cannot resolve Cofunction to a producer."]
resolveTerm CnsRep (CST.Lambda loc _fv _tm) =
  throwOtherError ["Cannot resolve Function to a consumer."]
---------------------------------------------------------------------------------
-- CST constructs which can only be resolved to commands
---------------------------------------------------------------------------------
resolveTerm _ (CST.Apply loc _ _) =
  throwOtherError ["Cannot resolve Apply command to a term."]
resolveTerm _ (CST.PrimCmdTerm _) =
  throwOtherError [" Cannot resolve primCmdTerm to a term."]
