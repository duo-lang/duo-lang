...
createSymbolTable' _ (ClassDecl MkClassDeclaration {classdecl_loc, classdecl_name, classdecl_xtors})  st = do
  let xtor_names = sig_name <$> classdecl_xtors
  ...
  pure $ st { classMethods = M.insert classdecl_name xtor_names (classMethods st)
            , xtorNameMap = M.union (M.fromList $ zip xtor_names (MethodNameResult classdecl_name . linearContextToArity . sig_args <$> classdecl_xtors)) (xtorNameMap st) }
