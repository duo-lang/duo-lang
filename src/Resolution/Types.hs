...
resolveTyp rep (TyNominal loc name args) = do
    ...
    case res of
        SynonymResult name' typ -> case args of
            ...
            _ -> throwOtherError ["Type synonyms cannot be applied to arguments (yet)."]
        NominalResult _ _ Refined _ -> do
            throwOtherError ["Refined type cannot be used as a nominal type constructor."]
...
associateOps lhs ((loc1, s1, rhs1) :| next@(loc2, s2, _rhs2) : rest) = do
    ...
    then do
        ...
        throwOtherError ["Unhandled case reached. This is a bug the operator precedence parser"]
