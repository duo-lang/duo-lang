...
instance AnnotateKind (RST.TypeScheme pol) (TST.TypeScheme pol) where
  ...
  annotateKind RST.TypeScheme {ts_loc = loc, ts_vars = tvs, ts_constraints, ts_monotype = ty} = do
    ty' <- annotateKind ty
    constrs <- mapM annotateKind ts_constraints
    newTVars <- mapM addTVar tvs
    return TST.TypeScheme {ts_loc = loc, ts_vars = newTVars, ts_constraints = constrs, ts_monotype = ty'}
    where
...
instance AnnotateKind RST.FreeConstraint TST.FreeConstraint where
  annotateKind :: RST.FreeConstraint -> GenM TST.FreeConstraint
  annotateKind (RST.SubTypeConstraint typ tyn) = do
