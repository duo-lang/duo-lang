...
genConstraintsTerm (Xtor loc rep xt@MkXtorName { xtorNominalStructural = Nominal } subst) = do
  ...
  case im of
    ...
    InferNominal -> do
      ...
      case rep of
        PrdRep -> return (Xtor (loc, TyNominal PosRep (Just (data_kind decl)) (data_name decl)) rep xt substInferred)
        CnsRep -> return (Xtor (loc, TyNominal NegRep (Just (data_kind decl)) (data_name decl)) rep xt substInferred)
    --
...
genConstraintsTerm (Cocase loc Nominal cocases@(MkTermCaseI {tmcasei_name = xtn}:_)) = do
  ...
  case im of
    ...
    InferNominal -> do
      ...
      return (Cocase (loc, TyNominal PosRep (Just data_kind) data_name) Nominal cocasesInferred)
    --
    -- Refinement Inference
    --
    InferRefined -> do
