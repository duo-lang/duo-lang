...
genConstraintsTerm (RST.Xtor loc rep Nominal xt subst) = do
  ...
  case rep of
    PrdRep -> return (AST.Xtor loc rep (Just (TyNominal PosRep Nothing (data_name decl) args)) Nominal xt substInferred)
    CnsRep -> return (AST.Xtor loc rep (Just (TyNominal NegRep Nothing (data_name decl) args)) Nominal xt substInferred)
--
-- Refinement Xtors
--
...
genConstraintsTerm (RST.XMatch loc rep Nominal cases@(pmcase:_)) = do
  ...
  case rep of
    PrdRep -> return $ AST.XMatch loc rep (Just (TyNominal PosRep Nothing (data_name decl) args)) Nominal (fst <$> inferredCases)
    CnsRep -> return $ AST.XMatch loc rep (Just (TyNominal NegRep Nothing (data_name decl) args)) Nominal (fst <$> inferredCases)
--
...
genConstraintsTerm (RST.Cocase loc Nominal cocases@(RST.MkTermCaseI {tmcasei_name = xtn}:_)) = do
  ...
  return (AST.Cocase loc (Just (TyNominal PosRep Nothing data_name args)) Nominal cocasesInferred)
--
