...
setType PosRep = topKwP *> return (TySet PosRep []) <|> TySet PosRep <$> (typP' PosRep) `sepBy2` unionSym
setType NegRep = botKwP *> return (TySet NegRep []) <|> TySet NegRep <$> (typP' NegRep) `sepBy2` intersectionSym
