...
class Infer a b | a -> b where
  infer :: ModuleName -> a -> b

instance Infer (Core.PrdCnsDeclaration pc) (DriverM (TST.PrdCnsDeclaration pc)) where
  infer :: ModuleName -> Core.PrdCnsDeclaration pc -> DriverM (TST.PrdCnsDeclaration pc)
  infer mn Core.MkPrdCnsDeclaration { pcdecl_loc, pcdecl_doc, pcdecl_pc, pcdecl_isRec, pcdecl_name, pcdecl_annot, pcdecl_term} = do
...
instance Infer Core.CommandDeclaration (DriverM TST.CommandDeclaration) where
  infer :: ModuleName -> Core.CommandDeclaration -> DriverM TST.CommandDeclaration
  infer mn Core.MkCommandDeclaration { cmddecl_loc, cmddecl_doc, cmddecl_name, cmddecl_cmd } = do
    env <- gets drvEnv
...
instance Infer Core.InstanceDeclaration (DriverM TST.InstanceDeclaration) where
  infer :: ModuleName -> Core.InstanceDeclaration -> DriverM TST.InstanceDeclaration
  infer mn decl@Core.MkInstanceDeclaration { instancedecl_loc, instancedecl_name, instancedecl_typ } = do
    env <- gets drvEnv
    -- Generate the constraints
...
instance Infer RST.ClassDeclaration (DriverM RST.ClassDeclaration) where
  infer :: ModuleName -> RST.ClassDeclaration -> DriverM RST.ClassDeclaration
  infer mn decl@RST.MkClassDeclaration { classdecl_name } = do
...
instance Infer Core.Declaration (DriverM TST.Declaration) where
  infer :: ModuleName -> Core.Declaration -> DriverM TST.Declaration
  --
  -- PrdCnsDecl
  --
  infer mn (Core.PrdCnsDecl pcrep decl) = do
    decl' <- infer mn decl
    pure (TST.PrdCnsDecl pcrep decl')
  --
  -- CmdDecl
  --
  infer mn (Core.CmdDecl decl) = do
    decl' <- infer mn decl
    pure (TST.CmdDecl decl')
  --
  -- DataDecl
  --
  infer mn (Core.DataDecl decl) = do
    -- Insert into environment
  ...
  -- XtorDecl
  --
  infer _mn (Core.XtorDecl decl) = do
  ...
  -- ImportDecl
  --
  infer _mn (Core.ImportDecl decl) = do
    pure (TST.ImportDecl decl)
  --
  -- SetDecl
  --
  infer _mn (Core.SetDecl CST.MkSetDeclaration { setdecl_option, setdecl_loc }) =
    throwOtherError setdecl_loc ["Unknown option: " <> setdecl_option]
  --
  -- TyOpDecl
  --
  infer _mn (Core.TyOpDecl decl) = do
    pure (TST.TyOpDecl decl)
  --
  -- TySynDecl
  --
  infer _mn (Core.TySynDecl decl) = do
    pure (TST.TySynDecl decl)
  --
  -- ClassDecl
  --
  infer mn (Core.ClassDecl decl) = do
    decl' <- infer mn decl
    pure (TST.ClassDecl decl')
  --
  -- InstanceDecl
  --
  infer mn (Core.InstanceDecl decl) = do
    decl' <- infer mn decl
...
inferProgram Core.MkModule { mod_name, mod_fp, mod_decls } = do
  let infer' :: Core.Declaration -> DriverM (Maybe TST.Declaration)
      infer' d = catchError (Just <$> infer mod_name d) (addErrorsNonEmpty mod_name Nothing)
  newDecls <- catMaybes <$> mapM infer' mod_decls
