...
inferInstanceDeclaration mn decl@Core.MkInstanceDeclaration { instancedecl_loc, instancedecl_class, instancedecl_typ } = do
  ...
  let f env = env { instancePosEnv = M.insert (instancedecl_class, instPos) instanceInferred (instancePosEnv env)
                  , instanceNegEnv = M.insert (instancedecl_class, instNeg) instanceInferred (instanceNegEnv env) }
  modifyEnvironment mn f
