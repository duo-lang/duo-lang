...
unsafeUnion (TypeAut polrep (Identity starts1) (TypeAutCore gr1)) 
            (TypeAut _      (Identity starts2) (TypeAutCore gr2)) =
...
typeAutEqual (TypeAut _ (Identity start1) (TypeAutCore gr1)) 
             (TypeAut _ (Identity start2) (TypeAutCore gr2))
  = case runStateT (typeAutEqualM (gr1, start1) (gr2, start2)) M.empty of
      Nothing -> False
      Just ((),mp) ->
        S.fromList (getFlowEdges gr2) ==
          S.fromList [(i',j') | (i,j) <- (getFlowEdges gr1), let Just i' = M.lookup i mp, let Just j' = M.lookup j mp]
