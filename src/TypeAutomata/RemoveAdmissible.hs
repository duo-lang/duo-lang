...
subtypeData aut@TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ (Just dat1) _ _ _) <- lab ta_gr i
  (MkNodeLabel Pos _ (Just dat2) _ _ _) <- lab ta_gr j
...
subtypeCodata aut@TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ _ (Just codat1) _ _) <- lab ta_gr i
  (MkNodeLabel Pos _ _ (Just codat2) _ _) <- lab ta_gr j
...
subtypeNominal TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ _ _ nominal1 _) <- lab ta_gr i
  (MkNodeLabel Pos _ _ _ nominal2 _) <- lab ta_gr j
...
subtypeRefined aut@TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ _ _ _ tyNames1) <- lab ta_gr i
  (MkNodeLabel Pos _ _ _ _ tyNames2) <- lab ta_gr j
