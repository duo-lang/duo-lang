...
subtypeData aut@TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg (Just dat1) _ _ _  _ _) <- lab ta_gr i
  (MkNodeLabel Pos (Just dat2) _ _ _ _ _) <- lab ta_gr j
...
subtypeCodata aut@TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ (Just codat1) _ _ _ _) <- lab ta_gr i
  (MkNodeLabel Pos _ (Just codat2) _ _ _ _) <- lab ta_gr j
...
subtypeNominal TypeAutCore{ ta_gr } (i,j) = do
  (MkNodeLabel Neg _ _ nominal1 _ _ _) <- lab ta_gr i
  (MkNodeLabel Pos _ _ nominal2 _ _ _) <- lab ta_gr j
