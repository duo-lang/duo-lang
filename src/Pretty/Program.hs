...
instance PrettyAnn (Declaration ext) where
  ...
  prettyAnn (FixityDecl _ (BinOp op LeftAssoc prec) tv1 tv2 typ) =
    annKeyword "infixl" <+> prettyAnn prec <+>
    prettyVarianceVar tv1 <+> prettyAnn op <+> prettyVarianceVar tv2 <+> 
    annSymbol ":=" <+> prettyAnn typ <> semi
  prettyAnn (FixityDecl _ (BinOp op RightAssoc prec) tv1 tv2 typ) =
...
instance PrettyAnn (NamedRep (Declaration ext)) where
  ...
  prettyAnn (NamedRep (ImportDecl _ mod)) =
    annKeyword "import" <+> prettyAnn mod <> semi
  prettyAnn (NamedRep (FixityDecl _ (BinOp op LeftAssoc prec) tv1 tv2 typ)) =
    annKeyword "infixl" <+> prettyAnn prec <+>
    prettyVarianceVar tv1 <+> prettyAnn op <+> prettyVarianceVar tv2 <+> 
    annSymbol ":=" <+> prettyAnn typ <> semi
  prettyAnn (NamedRep (FixityDecl _ (BinOp op RightAssoc prec) tv1 tv2 typ)) =
    annKeyword "infixr" <+> prettyAnn prec <+> 
    prettyVarianceVar tv1 <+> prettyAnn op <+> prettyVarianceVar tv2 <+>
    annSymbol ":=" <+> prettyAnn typ <> semi
