...
import Resolution.Unresolve (Reparse (..))
...
instance PrettyAnn (RST.VariantType pol) where
  prettyAnn varType = prettyAnn (reparse varType)

instance PrettyAnn (RST.PrdCnsType pol) where
  prettyAnn pctype = prettyAnn (reparse pctype)
...
instance {-# OVERLAPPING #-} PrettyAnn (RST.LinearContext pol) where
  prettyAnn ctxt = prettyAnn (reparse ctxt)
...
instance PrettyAnn (RST.XtorSig pol) where
  prettyAnn xtorSig = prettyAnn (reparse xtorSig)
...
instance PrettyAnn (RST.Typ pol) where
  prettyAnn typ = prettyAnn (reparse typ)
...
instance PrettyAnn (RST.TypeScheme pol) where
  prettyAnn tys = prettyAnn (reparse tys)
