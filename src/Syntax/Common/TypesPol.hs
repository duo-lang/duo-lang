...
data Typ (pol :: Polarity) where
  TySkolemVar :: Loc -> PolarityRep pol -> Maybe MonoKind -> TSkolemVar -> Typ pol
  TyUniVar :: Loc -> PolarityRep pol -> Maybe MonoKind -> TUniVar -> Typ pol
  ...
  -- | Equirecursive Types
  TyRec :: Loc -> PolarityRep pol -> TSkolemVar -> Typ pol -> Typ pol
  -- | Builtin Types
...
class FreeUniTVars (a :: Type) where
  freeUniTVars :: a -> Set TUniVar

instance FreeUniTVars (Typ pol) where
  freeUniTVars (TyUniVar _ _ _ tv)      = S.singleton tv
  freeUniTVars (TySkolemVar _ _ _ _)    = S.empty
  freeUniTVars TyTop {}                 = S.empty
  freeUniTVars TyBot {}                 = S.empty
  freeUniTVars (TyUnion _ _ ty ty')     = S.union (freeUniTVars ty) (freeUniTVars ty')
  freeUniTVars (TyInter _ _ ty ty')     = S.union (freeUniTVars ty) (freeUniTVars ty')
  freeUniTVars (TyRec _ _ _ t)          = freeUniTVars t
  freeUniTVars (TyNominal _ _ _ _ args) = S.unions (freeUniTVars <$> args)
  freeUniTVars (TySyn _ _ _ ty)         = freeUniTVars ty
  freeUniTVars (TyData _ _ _ xtors)     = S.unions (freeUniTVars <$> xtors)
  freeUniTVars (TyCodata _ _ _ xtors)   = S.unions (freeUniTVars <$> xtors)
  freeUniTVars (TyPrim _ _ _)           = S.empty
  freeUniTVars (TyFlipPol _ ty)         = freeUniTVars ty

instance FreeUniTVars (PrdCnsType pol) where
  freeUniTVars (PrdCnsType _ ty) = freeUniTVars ty

instance FreeUniTVars (VariantType pol) where
  freeUniTVars (CovariantType ty)     = freeUniTVars ty
  freeUniTVars (ContravariantType ty) = freeUniTVars ty

instance FreeUniTVars (LinearContext pol) where
  freeUniTVars ctxt = S.unions (freeUniTVars <$> ctxt)

instance FreeUniTVars (XtorSig pol) where
  freeUniTVars MkXtorSig { sig_args } = freeUniTVars sig_args


class FreeSkolemTVars (a :: Type) where
  freeSkolemTVars :: a -> Set TSkolemVar

instance FreeSkolemTVars (Typ pol) where
  freeSkolemTVars (TyUniVar _ _ _ _)      = S.empty
  freeSkolemTVars (TySkolemVar _ _ _ tv)   = S.singleton tv
  freeSkolemTVars TyTop {}                 = S.empty
  freeSkolemTVars TyBot {}                 = S.empty
  freeSkolemTVars (TyUnion _ _ ty ty')     = S.union (freeSkolemTVars ty) (freeSkolemTVars ty')
  freeSkolemTVars (TyInter _ _ ty ty')     = S.union (freeSkolemTVars ty) (freeSkolemTVars ty')
  freeSkolemTVars (TyRec _ _ v t)          = S.delete v (freeSkolemTVars t)
  freeSkolemTVars (TyNominal _ _ _ _ args) = S.unions (freeSkolemTVars <$> args)
  freeSkolemTVars (TySyn _ _ _ ty)         = freeSkolemTVars ty
  freeSkolemTVars (TyData _ _ _ xtors)     = S.unions (freeSkolemTVars <$> xtors)
  freeSkolemTVars (TyCodata _ _ _ xtors)   = S.unions (freeSkolemTVars <$> xtors)
  freeSkolemTVars (TyPrim _ _ _)           = S.empty
  freeSkolemTVars (TyFlipPol _ ty)         = freeSkolemTVars ty

instance FreeSkolemTVars (PrdCnsType pol) where
  freeSkolemTVars (PrdCnsType _ ty) = freeSkolemTVars ty

instance FreeSkolemTVars (VariantType pol) where
  freeSkolemTVars (CovariantType ty)     = freeSkolemTVars ty
  freeSkolemTVars (ContravariantType ty) = freeSkolemTVars ty

instance FreeSkolemTVars (LinearContext pol) where
  freeSkolemTVars ctxt = S.unions (freeSkolemTVars <$> ctxt)

instance FreeSkolemTVars (XtorSig pol) where
  freeSkolemTVars MkXtorSig { sig_args } = freeSkolemTVars sig_args

-- | Generalize over all free type variables of a type.
generalize :: Typ pol -> TypeScheme pol
generalize ty = TypeScheme defaultLoc (S.toList (freeUniTVars ty)) (S.toList (freeSkolemTVars ty)) ty

------------------------------------------------------------------------------
-- Bisubstitution and Zonking
------------------------------------------------------------------------------

newtype Bisubstitution = MkBisubstitution { uvarSubst :: Map TUniVar (Typ Pos, Typ Neg) }
