...
data Command ext
  ...
  --
  --   p >> c
  = Apply ext (Maybe CallingConvention) (STerm Prd ext) (STerm Cns ext)
...
openCommandComplete (Apply _ _ t1 t2) = Apply () Nothing (openSTermComplete t1) (openSTermComplete t2)
...
removeNamesCmd (Apply ext cc prd cns) = Apply ext cc (removeNamesSTerm prd) (removeNamesSTerm cns)
